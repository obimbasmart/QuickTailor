...
def upload_file():
    ...
    if file:
        ...
        file_url = s3_client.upload_single_photo(
            file, unique_id)  # Upload file to S3
...
def handle_send_message(data):
    ...
    if not message_text and not image_url:
        emit(
            'error', {'error': 'Message and image cannot  be empty'}, room=current_user.id)
        print(image_url)
        return
    ...
    if message_text and message_text.strip() == "":
        if image_url:
            ...
            emit(
                'error', {'error': 'Message cannot contain only spaces'}, room=current_user.id)
            return
    ...
    if current_user.is_tailor:
        ...
        check_msg = Message.query.filter_by(
            sender_user_id=reciever_id, reciever_tailor_id=current_user.id).all()
        if check_msg:
            ...
            msg_list = MessageList.query.filter_by(
                user_id=reciever_id, tailor_id=current_user.id).one_or_none()
        ...
        msg1 = Message.query.filter_by(reciever_user_id=current_user.id).filter_by(
            sender_tailor_id=reciever_id).all()
        msg2 = Message.query.filter_by(sender_user_id=current_user.id).filter_by(
            reciever_tailor_id=reciever_id).all()
        if not msg1 and not msg2:
            ...
            msg_list = MessageList.query.filter_by(
                user_id=current_user.id, tailor_id=reciever_id).one_or_none()
...
def messages():
    ...
    for m in msg_list:
        print("thi is upadted at: ", m.updated_at,
              "and this is created at", m.created_at)
    ...
    for msg_list in formatted_msg_list:
        msg_list.tailor.img_url = s3_client.generate_presigned_url(
            'get_object', msg_list.tailor.photo_url)
...
def handle_post(product_id):
    ...
    if request.method == 'POST':
        ...
        if product and not current_user.is_tailor:
            dat = {"id": product.id, "image": product.img,
            ...
            if request.is_json:
                ...
                msg1 = Message.query.filter_by(
                    reciever_tailor_id=product.tailor_id, sender_user_id=current_user.id).all()
                msg2 = Message.query.filter_by(
                    sender_tailor_id=product.tailor_id, reciever_user_id=current_user.id).all()
...
def messages_per_user(msg_id, modal):
    ...
    if current_user.is_tailor and not modal:
        ...
        msg1 = Message.query.filter_by(
            reciever_user_id=msg_id, sender_tailor_id=current_user.id).all()
        msg2 = Message.query.filter_by(
            sender_user_id=msg_id, reciever_tailor_id=current_user.id).all()
        ...
        if not modal:
            msg1 = Message.query.filter_by(
                reciever_tailor_id=msg_id, sender_user_id=current_user.id).all()
            msg2 = Message.query.filter_by(
                sender_tailor_id=msg_id, reciever_user_id=current_user.id).all()
            ...
            if product:
                ...
                msg2 = Message.query.filter_by(
                    sender_tailor_id=product.tailor_id, reciever_user_id=current_user.id).all()
                tailor = Tailor.query.filter_by(
                    id=product.tailor_id).one_or_none()
            else:
        ...
        if not msg1 and not msg2:
            ...
            if modal:
                ...
                for msg in json_message:
                    if msg.get('m_product'):
                        # print(msg)
                        del msg['m_product']['reviews']
                ...
                return jsonify({"current_user": m_user, "messages": json_message, "other_user": m_tailor})
